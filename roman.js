function RomanNumerals() {
	this.numeralToNumb = { 'I': 1, 'V': 5, 'X': 10, 'L': 50, 'C': 100, 'D': 500, 'M': 1000 };
	this.numberToNume =  { 1: 'I', 5: 'V', 10: 'X', 50: 'L', 100: 'C', 500: 'D', 1000: 'M' };
}

RomanNumerals.prototype.toRoman = function(number) {
  string = ('M').repeat(Math.floor(number / 1000));
	string += addNumeral(number, 100);
	string += addNumeral(number, 10);
	string += addNumeral(number, 1);
	return string;
}

function addNumeral(number, factor) {
	string = '';
	number = Math.floor(number / factor);
	let unit1 = RomanNumerals.numberToNume[factor];
	let unit5 = RomanNumerals.numberToNume[factor * 5];
	let unit10 = RomanNumerals.numberToNume[factor * 10];
	if (number % 10 == 9) {
		string = unit1 + unit10;
	} else if (number % 10 >= 5) {
		string = unit5;
...
RomanNumerals.prototype.fromRoman = function(numeral) {
	return numeral.split('').reverse().reduce((sum, num, i, arr) => {
		return sum +
					 this.numeralToNumb[num] *
		 		   ((this.numeralToNumb[num] > sum || arr[i] == arr[i-1]) ? 1 : -1);
	}, 0)
}
